...
<body class="min-h-screen">
  <div class="flex h-screen overflow-hidden">
    ...
    <div class="flex flex-col flex-1 bg-gray-50 overflow-auto">
      ...
      <!-- Main Content -->
      <main class="p-6">
        <div class="bg-white border border-gray-100 card">
          <!-- Card Header -->
          <div class="p-8 card-body">
            <div class="flex md:flex-row flex-col justify-between items-start md:items-center gap-6 mb-8">
              <div>
                <h6 class="mb-1 font-bold text-gray-900 text-3xl">Audit Trail & Transaction</h6>
            ...
            <div class="border border-gray-100 rounded-xl overflow-x-auto">
              <table class="table divide-y divide-gray-200">
                ...
                <thead>
                  <tr class="bg-gray-50">
                    <th class="px-6 py-3 font-medium text-gray-500 text-xs text-left uppercase tracking-wider">Log ID</th>
                    <th class="px-6 py-3 font-medium text-gray-500 text-xs text-left uppercase tracking-wider">Department</th>
                    <th class="px-6 py-3 font-medium text-gray-500 text-xs text-left uppercase tracking-wider">Employee</th>
                    <th class="px-6 py-3 font-medium text-gray-500 text-xs text-left uppercase tracking-wider">Modules</th>
                    <th class="px-6 py-3 font-medium text-gray-500 text-xs text-left uppercase tracking-wider">Action</th>
                    <th class="px-6 py-3 font-medium text-gray-500 text-xs text-left uppercase tracking-wider">Activity</th>
                    ...
                    <?php foreach ($trails as $trail): ?>
                      <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-gray-500 text-sm whitespace-nowrap">
                          ...
                          <div class="text-gray-400 text-xs">ID: <?php echo htmlspecialchars($trail['dept_id'] ?? ''); ?></div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                          <div class="flex items-center">
                            <div class="flex flex-shrink-0 justify-center items-center bg-gray-200 rounded-full w-10 h-10">
                    ...
                    <tr>
                      <td colspan="8" class="py-12 text-center">
                        <div class="flex flex-col justify-center items-center gap-4">
                          ...
                          <div class="space-y-1">
                            <h3 class="font-medium text-gray-900 text-lg">No audit trail records found</h3>
            ...
            <!-- Pagination -->
            <div class="flex sm:flex-row flex-col justify-between items-center gap-4 mt-6 pt-6 border-gray-100 border-t">
              <div class="text-gray-500 text-sm">
              ...
              <div class="join">
                <?php if ($page > 1): ?>
                  <a href="?page=<?php echo $page - 1; ?>" class="hover:bg-gray-50 border border-gray-200 join-item btn btn-sm btn-ghost">
                    <i data-lucide="chevron-left" class="w-4 h-4"></i>
                  </a>
                <?php else: ?>
                  <button class="hover:bg-gray-50 opacity-50 border border-gray-200 join-item btn btn-sm btn-ghost disabled">
                    <i data-lucide="chevron-left" class="w-4 h-4"></i>
                  </button>
                <?php endif; ?>
                ...
                if ($startPage > 1) {
                  echo '<a href="?page=1" class="hover:bg-gray-50 border border-gray-200 join-item btn btn-sm btn-ghost">1</a>';
                  if ($startPage > 2) {
                    echo '<button class="hover:bg-gray-50 opacity-50 border border-gray-200 join-item btn btn-sm btn-ghost disabled">...</button>';
                  }
                }

                for ($i = $startPage; $i <= $endPage; $i++) {
                  $active = $i == $page ? 'active' : '';
                  echo '<a href="?page=' . $i . '" class="hover:bg-gray-50 border border-gray-200 join-item btn btn-sm btn-ghost pagination-link ' . $active . '">' . $i . '</a>';
                }
                ...
                if ($endPage < $totalPages) {
                  ...
                  echo '<a href="?page=' . $totalPages . '" class="hover:bg-gray-50 border border-gray-200 join-item btn btn-sm btn-ghost">' . $totalPages . '</a>';
                }
                ?>
  ...
  <!-- Details Modal -->
  <div id="trailModal" class="modal-bottom modal sm:modal-middle">
    <div class="bg-white/90 max-w-2xl text-black modal-box">
      <h3 class="font-bold text-lg">Audit Trail Details</h3>
      <div class="space-y-4 py-4">
        <div class="gap-4 grid grid-cols-1 md:grid-cols-2">
          <div>
            <label class="block font-medium text-gray-500 text-sm">Log ID</label>
            <p class="mt-1 text-gray-900 text-sm" id="modal-id"></p>
          </div>
          <div>
            <label class="block font-medium text-gray-500 text-sm">Date</label>
            <p class="mt-1 text-gray-900 text-sm" id="modal-date"></p>
          </div>
          <div>
            <label class="block font-medium text-gray-500 text-sm">Department</label>
            <p class="mt-1 text-gray-900 text-sm" id="modal-dept"></p>
          </div>
          <div>
            <label class="block font-medium text-gray-500 text-sm">Employee</label>
            <p class="mt-1 text-gray-900 text-sm" id="modal-employee"></p>
          </div>
          <div>
            <label class="block font-medium text-gray-500 text-sm">Role</label>
            <p class="mt-1 text-gray-900 text-sm" id="modal-role"></p>
          </div>
          <div>
            <label class="block font-medium text-gray-500 text-sm">Action</label>
            <p class="mt-1 text-sm" id="modal-action"></p>
          </div>
        </div>
        <div>
          <label class="block font-medium text-gray-500 text-sm">Modules</label>
          <div class="flex flex-wrap gap-2 mt-1" id="modal-modules"></div>
        </div>
        <div>
          <label class="block font-medium text-gray-500 text-sm">Activity Details</label>
          <div class="bg-gray-50 mt-1 p-3 rounded-lg">
            <p class="text-gray-900 text-sm" id="modal-activity"></p>
          </div>
        </div>
      </div>
