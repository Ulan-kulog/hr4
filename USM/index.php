...
function logAttempt($usm, $Employee_ID, $Employee_name, $Role, $Log_Status, $Attempt_Type, $Attempt_Count, $Failure_reason, $Cooldown)
{
...
function logDepartmentAttempt($usm, $Department_ID, $employee_ID, $Name, $Role, $Log_Status, $Attempt_type, $Attempt_Count, $Failure_reason, $Cooldown_Until)
{
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && $employee_ID && $password) {
    ...
    // Core 2 Check
    $stmt = mysqli_prepare($usm, "SELECT email, employee_name, password, Dept_id, employee_id, role FROM department_accounts WHERE employee_id = ?");
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        if ($password === $row["password"]) {
            ...
            if (sendOTP($row["email"], $otp)) {
                logAttempt($usm, $employee_ID, $Name, $Role, 'Authenticating', 'Login', 0, 'Authenticating', '');
                logDepartmentAttempt($usm, $Department_ID, $employee_ID, $Name, $Role, 'Success', 'Login', 0, 'Login Successful', '');
                header("Location: 2fa_verify.php");
                exit();
            } else {
                logAttempt($usm, $employee_ID, $Name, $Role, 'Failed', 'Login', 0, 'Failed to send OTP email', '');
            ...
            logAttempt($usm, $employee_ID, $Name, $Role, 'Failed', 'Login', 0, 'Incorrect password', '');
